...
#define REPLIC_SIZE 3
#define MAX_HASH 0xFFFFFFFF

struct hash_ring_info {
    unsigned int server_hash;
    int server_index;
};
typedef struct hash_ring_info hash_ring_info;

struct load_balancer {
    server_memory *servers;
    hash_ring_info *hash_ring;
    int server_count;
};
